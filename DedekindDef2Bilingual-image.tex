...
\usepackage[margin=1in]{geometry} % default wide margins for single‐column pages
...
The famous definition of a set being finite appears in the text of Dedekind's 1888 \emph{Was sind und was sollen die Zahlen} (henceforth [WZ]), but the second definition---dating to 1889---was only published as an aside in the foreword to the second edition in 1893, as a potential alternative:
...
It may well be he was hoping to working up a version for publication, but things never panned out; it seems this is unknown.
At the very least Emmy-Lou Haffner has told me she does not think the manuscript was at the stage of being prepared for publication.

Even in the 1911 third edition of [WZ] Dedekind makes no edits to the text, adding only another small preface acknowledging criticism of his non-mathematical `proof' of Theorem 66, defending the `inner harmony' of the logic of his monograph, and asking the reader's indulgence for not having made updates including a `rigorous examination of the creative power of the mind' due to other work. 
So this second definition of finite set lay fallow for a couple of decades, as far as the public knew. 
However Dedekind had in fact worked out, as far as his naive set theory framework allowed, the relationship between his second definition of finite set, and the notion of finiteness that is ``of cardinality a natural number''.

The ``entirely different definition of the finite and the infinite'' is the subject of the notes translated below, which to my knowledge have only appeared in Dedekind's collected works \cite[Vol.~3, page~450]{Dedekind_Werke} edited by Robert Fricke, Emmy Noether and \O ystein Ore, in the original German. 

The note is followed by a short editorial commentary by Noether, and she cites the then-forthcoming note \cite{Cavailes_32} by Jean Cavaill\`es that fills in a gap in Dedekind's proof of the Lemma for Theorem 28 (Noether and Cavaill\`es would go on to co-edit the Cantor--Dedekind correspondence, published after Noether's death). 
Cavaill\`es notes that while the Lemma proves an inclusion in one direction, it doesn't 
Noether also includes quote from a letter Dedekind wrote to Weber on the matter.

Reference Kuratowski


%%%%%%
\bigskip

The note was published in \cite{Dedekind_Werke}, and included a pointer to \cite{Cavailes_32}, which filled a small gap in one of the proofs (the Lemma for the Corollary to Theorem 28), as well as providing the theorem that induction works as expected.

An interesting point to observe, aside from Dedekind's noting that the concept of injective mapping is not needed, is that this definition of finiteness is a \emph{positive} one ($\exists f$ such that [property holds]). 
...
In contrast the more famous Dedekind finiteness is a \emph{negative} one ($\not\exists f$ such that [property holds]) making the definition of `Dedekind finite' more like `not Dedekind infinite', whereas the negation of Dedekind's second defition now gives the concept of infinite set. 
And the notion of `finite set' according to this new definition do line up with the classical definitions, as follows.


% Place the orcid_logo.png file in the document directory for the ORCID icon below.
The following argument is based on one communicated to me by Jason Carr~\orcidlink{0009-0000-7254-9607}, using the following rephrasing of Dedekind's definition: A set $S$ is \emph{D2-finite} if there is an endomorphism $f$ of $S$ such that if $U\subseteq S$ is stable under $f$, and $\exists x \in U$, then $U=S$. 
...
An empty set is both D2-finite and classically finite, so we only consider the case that the set $S$ has a specified element.
...
Thus $c$ factors through the quotient map $\mathbb{N}\twoheadrightarrow \underline{m+1}$, to give a surjection $c'\colon \underline{m+1}\to S$, making $S$ the quotient of a classically finite set, what is called `K-finite' in the constructive mathematics literature. 
...
Choice-sensitive readers will notice that the Axiom of Choice was not used in the argument, as is highlighted by Noether in her commentary, so that a `Dedekind-style' definition of finiteness can be used in place of the classical definition, without, as Dedekind says ``without recourse to the natural numbers.''



Acknowledgements are due to Peter Luschny for building the two-column bilingual document structure and also helping finesse the translation after I made a first pass on rendering the software-translated English to something minimally acceptable.
I thank Jason Carr for discussion about the equivalence between Dedekind's definition and existing definitions.
...
Pursuit of this definition of a finite system $S$ \emph{without recourse to the natural numbers}.

...
The note is followed by a short editorial commentary by Noether, and she cites the then-forthcoming note \cite{Cavailes_32} Jean Cavaill\`es that (I believe) fills in a gap in the proof given by Dedekind (Noether and Cavaill\`es would go on to co-edit the Cantor--Dedekind correspondence, published after Noether's death). 
...
The following argument is based on communicated to me by Jason Carr\footnote{[ORCID]}, using the following rephrasing of Dedekind's definition: A set $S$ is \emph{D2-finite} if there is an endomorphism $f$ of $S$ such that if $U\subseteq S$ is stable under $f$, and $\exists x \in U$, then $U=S$. 
...
Such a set is classically finite in classical logic, but we can observe that the above proof (CHECK!) is actually constructively valid. 
Thus even in constructive mathematics an inhabited D2-finite set is (inhabited and) K-finite, and the ???converse is also true: consider $c\colon \underline{m+1}\twoheadrightarrow S$, and the function $\widetilde{f}\colon \underline{m+1} \stackrel{+1}{\to} \underline{m+1} \stackrel{c}{\twoheadrightarrow} S$. Let $k,\ell\in \underline{m+1}$ such that $c(k)=c(\ell)$. 

[[I want this function to descend to an endomorphism of $S$. Need: $c(k+1) = c(\ell+1)$]]

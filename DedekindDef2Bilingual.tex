\documentclass[leqno,hidelinks,10pt]{article}
\usepackage[german,english]{babel}
\usepackage{amsmath,amssymb,amsthm}
\usepackage{xcolor,hyperref}
\usepackage{graphicx} % For including graphics (e.g., ORCID logo)
...
\usepackage[margin=1.2in]{geometry} % default wide margins for single‐column pages
...
\newcommand{\DoubleCaptionFig}{% added <<<<<<<<<<<<<<<<<<<<< https://tex.stackexchange.com/a/211767/161015
\end{paracol}
    ...
    %% left cycle
    % ticks and labels
    \def\tickmarks{{90/$c$},{105/$c'$},{210/$b$},{225/$b'\ \,$},{320/$a$},{335/$a'$}}
    \foreach \angle/\ticklabel in \tickmarks {
        \draw[thick] (\angle:{\radius - 0.15cm}) -- (\angle:{\radius + 0.15cm});
        \node at (\angle:{\radius + 0.35cm}) {\ticklabel};}
    % arrow
    \def\startAngle{30}
    \def\endAngle{50}
    \def\arrowRadius{\radius + 0.5cm}
    \draw[->, >=Stealth, thick] (\startAngle:\arrowRadius) arc (\startAngle:\endAngle:\arrowRadius);
    \node[below] at ($(\startAngle:\arrowRadius) + (0.2cm, 0.1cm)$) {$\varphi$};
    \end{scope}
    \begin{scope}[shift={(4cm,-2.5cm)}]
    \draw (0,0) circle (\radius);
    %% right cycle
    % ticks and labels
    \def\tickmarks{{90/$b$},{105/$b'$},{210/$c$},{225/$c'\ \,$},{320/$a$},{335/$a'$}}
    \foreach \angle/\ticklabel in \tickmarks {
        \draw[thick] (\angle:{\radius - 0.15cm}) -- (\angle:{\radius + 0.15cm});
        \node at (\angle:{\radius + 0.35cm}) {\ticklabel};}
    % arrow
...
% \title{Second definition of the finite and infinite (1889.3.9)}
...
%% CHANGE MARGINS FOR MY SECTION! THEY ARE TOO NARROW
% \begin{abstract}
% The current document aims to make better known another definition of finiteness introduced by Dedekind, in addition to his well-studied eponymous \emph{Dedekind finiteness}, by providing an English translation of the posthumously-published document that establishes the definition's consequences.
%     \end{abstract}


% \newpage
\newgeometry{total={8in,10in}}

\title{Zweite Definition (1889.3.9) des Endlichen und Unendlichen.\\
{\normalsize Second definition (1889.3.9) of the finite and infinite}}
\author{Richard Dedekind}
\date{}

\maketitle

% switch to narrower layout for two‐column pages
...
Now to pursue this definition of a finite system $S$ \emph{without recourse to the natural numbers}.
...
element $h$ in $H_s$. If $k=h'$ is different from $s'$, then $h$ is also different
...
If $A + B$ means the system composed of $A$ and $B$, then one has
\[
    a'b + b'a = S.
\]%
\beweis
...
\[
    A = c'b,\ B = a'c,\ C = b'a;\  A_1 = b'c,\ B_1 = c'a,\ C_1 = a'b,
\]
then $A - B - C = 0$ (empty) (here the symbol $-$ means intersection) and
...
erklärte, (nach 3) in $T$ liegende Element $s_1$ übergeht. Ist dann
...
\noindent obgleich $1 < r + n - s < n$. Dass endlich $S$ keine anderen als diese $n$ Elemente
...
But if $H_s$ is a proper part of $S$, then let $A$ be the system of all elements
...
To prove this, we first provide the following Lemma:
...
\Beweis
Were $k=s$, then $hs=s's= S$, whereas $hs$ is a proper part of $as$, and hence also of $S$. Thus, since $k$ is [necessarily] different from $s$, then (by \sref{thm18}) $ks = [k]+ hs$,
...
\end{enumerate}
\beweis
Since $s's = S$, and therefore $T \partof s's$ (by \sref{thm15}), there is at least
...
$s_1$ has property 1., but also property 2., because $T$ is a common part of
...
$\psi$ into the element $s_1$, which is defined there and (according to \sref{thm29}.3)
...
all elements of $S$, and therefore be $= S$. \QED''\footnote{[\emph{cf Brief an Weber 11 Jan. 1906. \cite[p.299]{Dedekind-Weber-letters}.
The passage quoted by Noether is a splicing together of a part of the letter in \emph{ibid}.\ and the notes that Dedekind included (``I am enclosing a sheet with comments'') for Weber to use to improve his manuscript ``Elementare Mengenlehre. Natürliche Zahlen''}.
]}
\end{quote}
...
\end{paracol}
% restore default margins for subsequent single‐column pages
...
\begin{center}{\Large\textbf{Commentary (VERY ROUGH DRAFT: IGNORE FOR NOW)\ \\ }}
\end{center}
...
Dedekind's famous definition of a set being finite appears in \S 64 of \emph{Was sind und was sollen die Zahlen} (\cite{WZ}, henceforth WZ), published in 1888.
But his \emph{second} definition, dating to 1889, was only published as an aside in the foreword to the 1893 second edition of WZ as a potential alternative:\footnote{I quote here from Beman's 1901 `authorised translation' of WZ.}

\begin{quote}
 For the benefit of those who are interested in and understand the difficulties of such an investigation, I add the following remark. We can lay down an entirely different definition of the finite and infinite, which appears still simpler since the notion of similarity of transformation [i.e.\ injectivity of a function] is not even assumed, viz.:
 \begin{quote}
 ...
 \end{quote}
 Now let us attempt to erect our edifice upon this new foundation!
 We shall soon meet with serious difficulties, and I believe myself warranted in saying that the proof of the perfect agreement of this definition with the former can be obtained only (and then easily) when we are permitted to assume the series of natural numbers as already developed and to make use of the final considerations in (131); and yet nothing is said of all these things in either the one definition or the other!
 From this we can see how very great is the number of steps in thought needed for such a remodeling of a definition.
\end{quote}

Dedekind subsequently developed the consequences of his definition; the manuscript %https://mathstodon.xyz/@estruppi@sciences.re/112195178889922135)
refers to the 1893 edition of WZ in its first sentence.
It may well be he was hoping to work up a version for publication, but things never panned out, and it seems the facts of the matter are unknown.
At the very least, the development of the results is incomplete, with the proof of the Lemma after Theorem 28 missing the conclusion of the argument, and the logical next step of the theory after Theorem 31---that the subset $T$ satisfies the new definition of `finite'---is not covered.
Emmy-Lou Haffner~\orcidlink{0000-0003-3210-9804} has told me she does not think the manuscript was at the stage of being prepared for publication.

Even in the 1911 third edition Dedekind makes no edits to the text, adding only another small preface acknowledging criticism of his non-mathematical `proof' of WZ's Theorem 66, defending the `inner harmony' of the logic of his monograph, and asking the reader's indulgence for not having made updates including a `rigorous examination of the creative power of the mind' due to other work.
So this second definition of finite set lay fallow for a couple of decades, as far as the public knew.
However Dedekind had in fact worked out, as far as his naive set theory framework allowed, the relationship between his second definition of finite set, and the notion of finiteness that is ``of cardinality a natural number'', and communicated this to Weber in 1906 [DETAILS?]

% The ``entirely different definition of the finite and the infinite'' is the subject of the notes translated here, which to my knowledge have only appeared in Dedekind's collected works \cite[Vol.~3, page~450]{Dedekind_Werke} edited by Robert Fricke, Emmy Noether and \O ystein Ore, in the original German.

The published version of note is followed by a short editorial commentary by Emmy Noether, and she cites the then-forthcoming note \cite{Cavailes_32} by Jean Cavaill\`es that fills in a gap in Dedekind's proof of the Lemma for Theorem 28.\footnote{Noether and Cavaill\`es would go on to co-edit the Cantor--Dedekind correspondence, published after Noether's unexpectedly early death.}
The proof of the Lemma as written doesn't establish the claim $ks\subseteq as$, as Dedekind doesn't stops in his proof before establishing one can take the non-specific element $m$ to be $k$ itself (in the notation of the proof). \\
Noether also includes quote from a letter Dedekind wrote to Weber on the matter.

Reference Tarski's paper \cite[\S 7]{Tarski_finite} (`De l'ordre cyclique. La seconde d\'efinition de Dedekind et la d\'efinition de M.\ Zermelo')

Noether's quoting Dedekind does not match the extant draft of the letter in R. Dedekind MS, published as \cite[p.\ 299]{Dedekind-Weber-letters}. The


Orbitals cf work of B\'enabou--Loiseau \cite{Benabou-Loiseau_94}. It seems that B--L show that a D2-finite set is K-finite in the internal logic of the ambient topos.


Mike Shulman's comments on Zulip etc (Freyd? \cite{Freyd_06})
...
Dedekind's note translated here was published in \cite[\S LXIII.]{Dedekind_Werke}, and included a pointer to \cite{Cavailes_32}, which filled a small gap in one of the proofs (the Lemma for the Corollary to Theorem 28), as well as providing the theorem that induction works as expected.

An interesting point to observe, aside from Dedekind's noting that the concept of injective mapping is not needed, is that this definition of finiteness is a \emph{positive} one: $\exists f$ such that [a certain property holds].
To be fair, the explicit statement given by Dedekind has a `no proper part' condition, but the condition can be rephrased as ``if $U\subseteq S$ is stable under $f$, then $\exists x\in U \Rightarrow U=S$'' (see below).
In contrast the more famous Dedekind finiteness is a \emph{negative} one ($\not\exists f$ such that [a certain property holds]) making the definition of `Dedekind finite' more like `not Dedekind infinite', whereas the negation of Dedekind's second definition now gives the concept of infinite set.
And the notion of `finite set' according to this new definition does line up with the classical definitions, as follows.


% Place the orcid_logo.png file in the document directory for the ORCID icon below.
The following argument is based on one communicated to me by Jason Carr~\orcidlink{0009-0000-7254-9607}, using the following rephrasing of Dedekind's definition: Call a set $S$ \emph{D2-finite} if there is an endomorphism $f$ of $S$ such that if $U\subseteq S$ is stable under $f$, and $\exists x \in U$, then $U=S$.
It's clear that any set that is in bijection with $\underline{k}:=\{1,\ldots k\}$ (i.e. a classically finite set) satisfies this definition, because we can take $f$ to be $n\mapsto n+1\pmod{k}$. But we can also show that D2-finiteness of $S$ implies $S$ is classically finite.

An empty set is both D2-finite and classically finite, so we only consider the case that the set $S$ has a specified element $x$.
Dedekind in any case only considered inhabited sets.


Given any subset $U_0\ni x$ (say) of $S$ and endomorphism $f$, we can take the union over the sequence $U_{i+1}:=f(U_i)$ of subsets of $S$, to get $U\subseteq S$, which contains $x$.
We have that $f$ restricts to an endomorphism of $U$, so then $U=S$.
But $U_0$ was arbitrary, so we could take it to be $U_0=\{x\}$ for some $x\in S$, and then for every $y\in S$, there exists some $n \geq 0$ such that $y=f^n(x)$ ($f^n = f\circ \cdots \circ f$, $n$ times).
As a result, the function $c\colon \mathbb{N}\to S$, $c(i) = f^i(x)$ and $c(0)=x$ is surjective.
Now consider $U_0 = \{c(1)\}$, so that there exists some $m$ such that $x=f^m(c(1)) = f^{m+1}(x)$.
Thus $c$ factors through the quotient map $\mathbb{N}\twoheadrightarrow \underline{m+1}$, to give a surjection $c'\colon \underline{m+1}\twoheadrightarrow S$, making $S$ the quotient of a classically finite set, what is called \emph{finitely indexed} in the constructive mathematics literature (in the presence of $\mathbb{N}$, this is equivalent to \emph{Kuratowski} or \emph{K}-finite).
Such a set is classically finite in classical logic, but we can observe that the above proof is actually constructively valid.
Thus even in constructive mathematics an inhabited D2-finite set is (inhabited and) finitely indexed.


Choice-sensitive readers will notice that the Axiom of Choice was not used in the argument, as is highlighted by Noether in her commentary.
Thus, unlike the usual notion of Dedekind finiteness, we have here a Choice-free `Dedekind-style' definition of finiteness that can be used in place of the classical definition, without, as Dedekind says ``recourse to the natural numbers''.



\textbf{Acknowledgements} are due to Peter Luschny~\orcidlink{0000-0001-6245-708X} for building the two-column bilingual document structure and also helping finesse the translation after I made a first pass on rendering the software-translated English to something minimally acceptable.
I thank Jason Carr for discussion about the equivalence between Dedekind's definition and existing definitions, and Emmylou Haffner for additional information, including about Dedekind's 1906 letter and notes sent to Weber.

Lastly, I thank the American Mathematical Society for granting permission to reproduce Emmy Noether’s original commentary on the chapter titled ``\emph{Zweite Definition (1889.3.9) des Endlichen und Unendlichen.}'' This material was originally published in `Gesammelte mathematische Werke, Volume II', edited by Robert Fricke, Emmy Noether, and \O ystein Ore, and reprinted by Chelsea Publishing Company, New York, in the 1969 edition. We are grateful for the AMS’s support in making this historically significant text more accessible.

\bigskip

% Right align this!
---David Michael Roberts, Adelaide, 2025

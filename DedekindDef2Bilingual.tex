...
also haben (nach \sref{gthm20}) die Strecken $a's$, $s'a$ kein gemeinsames
\newpage
\noindent Element, und (nach \sref{gthm17}) ist  $a's + s'a = S = H_s + A$, mithin $A \TeilVon s'a$.
Nimmt man nun an, es sei $a's$ ein echter Teil von $H_s$, und bezeichnet mit $H$ das System aller derjenigen Elemente von $H_s$, welche außerhalb $a's$, also in
...
\begin{center}
{\Large\textbf{Erläuterungen zur vorstehenden Abhandlung}}
\end{center}
...
different, so (by \sref{thm20}) the segments $a's$, $s'a$ have no common  \newpage
...
system 
 of all those elements of $H_s$ which are outside $a's$, i.e., in $s'a$,

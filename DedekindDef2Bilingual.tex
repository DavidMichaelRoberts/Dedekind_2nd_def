...
\Beweis
If $k=s$, then $hs=s's= S$, whereas $hs$ is a proper part of $as$, and hence also of $S$. Since $k$ is different from $s$, then (by \sref{thm18}) $ks = [k]+ hs$,
...
Orbitals cf work of B\'enabou--Loiseau \cite{Benabou-Loiseau_94}, Mike Shulman's comments on Zulip etc (Freyd? \cite{Freyd_06})
...
Thus $c$ factors through the quotient map $\mathbb{N}\twoheadrightarrow \underline{m+1}$, to give a surjection $c'\colon \underline{m+1}\twoheadrightarrow S$, making $S$ the quotient of a classically finite set, what is called \emph{K-finite} or \emph{subfinite} in the constructive mathematics literature. 
...
Thus even in constructive mathematics an inhabited D2-finite set is (inhabited and) K-finite.
